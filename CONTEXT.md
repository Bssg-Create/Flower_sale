...
## 2026-08-16（零基础答辩学习材料第一批完成）

### 用户批准与本轮范围
- 用户已明确批准《零基础项目熟悉与答辩学习材料制作方案》，并指定先制作第一批材料。
- 本轮只新增 `docs/答辩学习材料/` 下三份 Markdown 学习材料，没有修改 Java/Vue 业务代码、数据库、接口、路由、IDEA 运行配置、前端构建产物或 8081 答辩包。
- 没有启动或停止任何服务，没有安装或下载依赖，没有读取 `.idea/flower-sales-dev.env` 内容，也没有记录账号、密码、Token、JWT 密钥、数据库凭据或密码哈希。

### 已完成的第一批材料
1. `docs/答辩学习材料/01-学习导航与进度检查表.md`
   - 给出 24 周学习路线、第一阶段检查表、口头验收题和后续学习卡制作顺序。
2. `docs/答辩学习材料/02-项目总览与目录结构零基础讲解.md`
   - 用鲜花店类比解释前端、四个 Maven 模块和 MySQL，记录当前 68 个 Java、16 个 Vue、4 个前端 `src` JavaScript 文件的真实统计，并说明当前真实 DIY 路由组件。
3. `docs/答辩学习材料/03-FlowerApplication启动流程与端口说明.md`
   - 解释 `FlowerApplication` 启动链、npm/Vite 最小概念、三个当前真实 IDEA 共享运行配置、5173/8081 分工、Vite 代理和 8081 单端口答辩原理。

### 只读依据与验证结果
- 制作前只读核对根/模块 POM、`FlowerApplication.java`、`application.yml`、`package.json`、`vite.config.js`、`main.js`、路由、Axios 配置、`SpaController`、`WebMvcConfig`、三个 `.run` 配置和后端 `static/index.html`。
- 当前真实基础为 Spring Boot `3.5.11`、Java `17`、MyBatis-Plus `3.5.5`；Vite 开发端口为 `5173`，Spring Boot 端口为 `8081`，`/api` 和 `/images` 在开发模式下由 Vite 转发到 8081。
- 当前共享运行配置确认为 `Flower Backend - Dev`、`Flower Frontend - Dev`、`Flower Full Stack - Dev`。材料统一说明：日常源码开发运行 Full Stack 并访问 5173；使用已封版静态资源答辩时运行 Backend 并访问 8081，不新建缺少本机环境配置的临时启动项。
- 三份文档完整性、标题、代码围栏、关键内容和引用源码路径检查均通过；`git diff --check` 通过；文档敏感值形态扫描未发现疑似密码、Token、JWT 密钥或带凭据数据库地址。

### 下一步
1. 先由用户阅读第一批三份材料，完成第一阶段口头验收题和进度检查表；不要求一次读完全部源码。
2. 用户确认第一批易懂且准确后，再提交下一批材料的精确制作范围；建议首先制作《页面到数据库的统一调用链》，然后制作《登录、JWT 与权限学习卡》。
3. 后续每张核心功能学习卡继续逐项引用真实页面、Vue 组件、API、Controller、Service、Mapper、数据库表、状态变化、事务/库存影响、权限边界和常见答辩问题。
4. 技术项目继续封版，不新增功能、不重做设计、不恢复 `92b5b80`；只有发现明确缺陷时才另行提出完整最小方案并等待用户批准。
